...
import { shortQuestionMap } from './mappers/shortQuestionMap';
import { pushAggregatesQuestionData } from './functions/pushAnswerAggregatesToQuestionData';
...
parseXlsxToJson().then(async (entries) => {
    ...
    // outputPath = path.join(__dirname, '..', 'json', 'answers.json');
    ...
    // await updateQuestions(entries.questionData);  // Uncomment this to push questionData
    // await pushAggregatesQuestionData(entries.answersAggregates, 'uuid-of-promedio');
...
// (async () => {
//     await updateQuestions(shortQuestionMap);
// })();
